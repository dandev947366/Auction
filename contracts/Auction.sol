   ...
   //Structure to hold the details of a persons
    ...
    function bid(uint _itemId, uint _count) public payable{
        ...
        */
        ...
        // ** Start code here. 2 lines approximately. **/
        if(_count > tokenDetails[msg.sender].remainingTokens)
            revert("Remaining Tokes are less than token bidded");
        if(_itemId > 2)
            revert("Can not bid for more than 2 items.");
    ...
    modifier onlyOwner {
        // ** Start code here. 2 lines approximately. **
         if(msg.sender != beneficiary){
            revert("Address needs to be contract owner");
        }
        _;
        //** End code here. **
    }
